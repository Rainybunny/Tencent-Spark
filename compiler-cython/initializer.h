 ...
 **/
Schedule readScr(const CircuitFile& cf) {
    Schedule res;
    assert(!(cf.size() % 3)); // name | subj | param
    for (int i = 0; i < (int)cf.size(); i += 3) {
        Gate cur; int siz;
        cur.name = cf[i];
        std::stringstream scan(cf[i + 1]);
        scan >> siz, cur.subj.resize(siz);
        for (int& s: cur.subj) scan >> s;
        scan = std::stringstream(cf[i + 2]);
        scan >> cur.param;
...
Schedule cancelSingle(Schedule sch) {
    size_t nsiz = sch.size();
    std::vector<std::vector<int>> sgate(13);
...
Graph buildGraph(const Schedule& sch) {
    Graph G(sch.size());
    std::vector<int> lastRef(13, -1);
    for (int i = 0; i < (int)sch.size(); ++i) {
        Gate cur = sch[i];
        assert(1 <= cur.subj.size() && cur.subj.size() <= 2);
        if (cur.subj.size() == 1) {
...
void init(const CircuitFile& cf, Schedule& sch, Graph& grp) {
    auto scr = readScr(cf);
    sch = cancelSingle(translate(scr));
#ifdef GATE_LOG
